...
async function logEvent(req, res) {
    try {
        ...
        const placeholders = validEvents.map((event, index) => {
            const base = index * 8;
            values.push(userId, guestId, event.content_id, event.event_type, session_id ?? null, event.position ?? null, event.metadata ? JSON.stringify(event.metadata) : null, event.dwell_time_ms ?? null);
