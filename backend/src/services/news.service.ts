...
function recencyScore(publishedAt: string): number {
  ...
  const hoursAgo = Math.max(0, (Date.now() - published) / 3_600_000)
  return Math.exp(-hoursAgo / 42)
}

function preferenceScore(raw: number): number {
...
function getImpressionWeight(dwellTimeMs?: number | null): number {
  if (!dwellTimeMs) return 0
  if (dwellTimeMs < 1200) return -0.2
  if (dwellTimeMs < 3000) return 0
  if (dwellTimeMs < 6000) return 0.18
  if (dwellTimeMs < 12000) return 0.32
  return 0.45
}

function getEventWeight(eventType: string, dwellTimeMs?: number | null): number {
  switch (eventType) {
    case 'swipe_right':
      return 1.15
    case 'save':
...
function interleaveUnique(groups: Article[][]): Article[] {
...
function applyDiversity(feed: Article[], limit: number = feed.length): Article[] {
  const result: Article[] = []
  const usedIds = new Set<string>()
  ...
  const pushArticle = (article: Article) => {
  ...
  while (result.length < limit && usedIds.size < feed.length) {
    ...
    for (const article of feed) {
      ...
      const wouldMakeTripleCategory =
        lastTwoCategories.length === 2 &&
        lastTwoCategories.every(itemCategory => itemCategory === category)

      if (wouldMakeTripleCategory) continue
      if ((categoryCounts[category] ?? 0) >= maxPerCategory) continue
      if ((sourceCounts[source] ?? 0) >= maxPerSource) continue
...
async function getUserLocation(userId: string): Promise<string | null> {
  const result = await pool.query<{ location: string | null }>(
    `SELECT location FROM app_users WHERE id = $1 LIMIT 1`,
    [userId]
  )
...
async function getTrendingScores(articleIds: string[]): Promise<Record<string, number>> {
  if (!articleIds.length) return {}

  const result = await pool.query<{ content_id: string; weighted_score: string }>(
    `
    SELECT
      content_id,
      SUM(
...
async function getRecentSeenCategoryCounts(userId: string): Promise<Record<string, number>> {
...
function computeAdaptiveScore(
  ...
  recentSeenCounts: Record<string, number>,
...
export async function getNews(
  ...
  const unseenCandidatesResult = await pool.query<Article>(
    `
  ...
  if (!candidates.length) {
    const fallbackResult = await pool.query<Article>(
      `
      SELECT id, title, summary, image_url, url, source, published_at, category, country
      FROM articles
      WHERE ${READY_ARTICLE_WHERE}
